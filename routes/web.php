...
Route::middleware(['auth', 'verified', 'role:instructor,admin'])->prefix('instructor')->name('instructor.')->group(function () {
    ...
    Route::post('/lessons/{lessonId}/quizzes/generate', [InstructorQuizController::class, 'generateFromAI'])->name('quizzes.generate');
    ...
    Route::get('/students/{id}/message', [Instructor\MessageController::class, 'index'])->name('students.message');
    Route::post('/students/{id}/message', [Instructor\MessageController::class, 'store'])->name('students.message.send');
