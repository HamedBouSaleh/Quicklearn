...
// Map question types from database to frontend
...
const form = ref({
    ...
    time_limit: props.quiz.time_limit || null,
    passing_score: props.quiz.passing_score,
    allow_retake: props.quiz.allow_retake !== undefined ? props.quiz.allow_retake : true,
...
const submit = () => {
    ...
    router.put(route('instructor.quizzes.update', props.quiz.id), form.value, {
        preserveScroll: true,
        onFinish: () => {
...
const cancel = () => {
    router.visit(route('instructor.quizzes.show', props.quiz.id));
};
</script>
...
<template>
    ...
    <InstructorLayout>
        <div class="max-w-4xl mx-auto">
            <div class="mb-6">
                <Link :href="route('instructor.quizzes.show', quiz.id)" class="text-primary-600 hover:text-primary-700 font-medium">
                    ← Back to Quiz
                </Link>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-8">
                ...
                <p class="text-gray-600 mb-8">Update quiz details and questions</p>
